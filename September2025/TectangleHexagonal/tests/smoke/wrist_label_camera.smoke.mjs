// Smoke test: camera_landmarks_wrist_label_v1.html
// Strategy: mock getUserMedia using a hidden <video> element's captureStream from a golden MP4.
// Assert that FPS > 0 within a few seconds (overlay draw may depend on model presence, so HUD FPS is a safe guard).
...
const outSummary = process.env.OUT || 'September2025/TectangleHexagonal/out/wrist_label.smoke.summary.json';
...
(async () => {
  ...
  await page.evaluateOnNewDocument((url) => {
    // Install a getUserMedia shim that uses captureStream from a hidden video
  ...
  // Wait for FPS > 0 within 8 seconds
